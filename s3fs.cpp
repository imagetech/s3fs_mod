...
static int makeMissingObject(const char* path) ;
//-------------------------------------------------------------------
...
static int get_object_attribute(const char* path, struct stat* pstbuf, headers_t* pmeta, bool overcheck, bool* pisforce, bool add_no_truncate_cache)
{
    ...
    S3FS_PRN_INFO3("Check cache for: [path=%s]", path);
    // Check cache.
    ...
    if(StatCache::getStatCacheData()->IsNoObjectCache(strpath)){
    	...
    	S3FS_PRN_INFO3("there is the path in the cache for no object: [strpath=%s]", strpath.c_str());
        return -ENOENT;
    }
    S3FS_PRN_INFO3("Finished NO cache for: [path=%s]", path);
    ...
    // 
    // DDL +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // Should end here when the directory is not found
    // Normally wants to add to the "no object" cache
    // Try not adding bdlnut stil returning
    //if( ! FIRST_CACHE_PASS && ( 0 != result && -EPERM != result) ){
    if(0 != result && -EPERM != result){
        // finally, "path" object did not find. Add no object cache.
        S3FS_PRN_INFO3("object did not find. NORMALLY Add no object cache -skip and return [path=%s]", strpath.c_str());
		return -ENOENT;
    ...
    if(std::string::npos != (Pos = strpath.find("_$folder$", 0))){
        S3FS_PRN_INFO3("path has _$folder$, need to cut it.[path=%s]", strpath.c_str());
        strpath.erase(Pos);
        strpath += "/";
    }
    ...
    //if( ! FIRST_CACHE_PASS && ( add_no_truncate_cache || 0 != StatCache::getStatCacheData()->GetCacheSize() ) ){
    if(add_no_truncate_cache || 0 != StatCache::getStatCacheData()->GetCacheSize()){
        // add into stat cache
        S3FS_PRN_INFO3("adding stat cache [path=%s]", strpath.c_str());
        ...
        if(!StatCache::getStatCacheData()->GetStat(strpath, pstat, pheader, overcheck, pisforce)){
        	S3FS_PRN_INFO3("not in cache.(why?) -> retry to convert [path=%s]", strpath.c_str());
        ...
        S3FS_PRN_INFO3("FIRST PASS or cache size is Zero -> only convert [path=%s]", strpath.c_str());
        // cache size is Zero -> only convert.
        if(!convert_header_to_stat(strpath.c_str(), (*pheader), pstat, forcedir)){
            S3FS_PRN_ERR("failed convert headers to stat[path=%s]", strpath.c_str());
            S3FS_PRN_INFO3("failed convert headers to stat[path=%s]", strpath.c_str());
            return -ENOENT;
        }
    }
    return 0;
...
static int makeMissingObject(const char* path) {
	...
	std::string fullpathcmad;
	std::string misspath(path);
	//aws s3api put-object --bucket some-bucket --key some/nested/folder/you/want/to/create
	std::string fullpathcmd("aws s3api put-object --bucket dev-wefx-assets-43249er329daasd02oivoqovjsvzx13 --key Volumes");
   	fullpathcmd.append(misspath);
   	S3FS_PRN_INFO1(" **get_object_attribute: OPTION to  MAKE DIRECTORY:%s", fullpathcmd.c_str());
            
    std::string endingDir("/");
    if ( endsWith(fullpathcmd.c_str(), endingDir.c_str())){
        S3FS_PRN_INFO3("fullpathcmd already ends with / %s", fullpathcmd.c_str());
    } else {
        S3FS_PRN_INFO3("fullpathcmd does NOT end with / %s", fullpathcmd.c_str());
        //S3FS_PRN_INFO3("This is a file so end of path - set to DDLsync %s", fullpathcmd.c_str());
        fullpathcmd.append(endingDir);
    
   	S3FS_PRN_INFO2("**DDL get_object_attribute: YES MAKE DIRECTORY:%s", fullpathcmd.c_str());
        //
    }

    const int dir_err = system(fullpathcmd.c_str());
    if (-1 == dir_err)
    {
    	S3FS_PRN_INFO("**Error creating directory!n");
    }
    else
    {
    	S3FS_PRN_INFO("**system mkdir returned:%d", dir_err);
   	} 
    //
    S3FS_PRN_INFO("**DONE MADE directory:%s", fullpathcmd.c_str());
...
static int s3fs_getattr(const char* _path, struct stat* stbuf)
{
	...
	FIRST_CACHE_PASS = true;
    // check parent directory attribute.
    if(0 != (result = check_parent_object_access(path, X_OK))){
        return result;
    }
    if(0 != (result = check_object_access(path, F_OK, stbuf))){
        S3FS_PRN_INFO("DDL check_object_access RESULT:%d [path=%s] uid=%u, gid=%u, mode=%04o", result, path, (unsigned int)(stbuf->st_uid), (unsigned int)(stbuf->st_gid), stbuf->st_mode);
		// didn't find so call API to handle
        S3FS_PRN_INFO("DDL s3fs_getattr call MakeMissingObject path=%s", path);


		// resolve the missing object
		S3fsCurl s3fscurlAPI;
		std::string api_call_msg("404 File Not Found");
		s3fscurlAPI.SendToSyncAPI(api_call_msg, path);
		//makeMissingObject(path); 
		s3fscurlAPI.DestroyCurlHandle();

		// call the check object again
		FIRST_CACHE_PASS = false;
    	result = check_object_access(path, F_OK, stbuf);
    	if(0 != result) {
    	   	S3FS_PRN_INFO("DDL s3fs_getattr STILL error (%d) after mkdir path=%s", result, path);
